...
export const createTestProgram = (overrides: Partial<TestProgram> = {}): TestProgram => ({
...
export const insertTestProgram = async (programData: Partial<TestProgram> = {}) => {
...
export const insertMultipleTestPrograms = async (count: number = 5) => {
  ...
  const insertedPrograms = await db
    .insert(programs)
    .values(programsData.map(program => ({
      name: program.name,
...
export const createValidProgramPayload = (overrides: Partial<TestProgram> = {}) => {
