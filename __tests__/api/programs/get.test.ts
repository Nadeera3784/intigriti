...
import { cleanupTestData, insertMultipleTestPrograms, insertTestProgram } from '../../utils/test-helpers'
...
describe('/api/programs GET', () => {
  ...
  describe('Sorting', () => {
    beforeEach(async () => {
      await insertTestProgram({ name: 'Alpha Program', eligibility: 'eligible' })
      await insertTestProgram({ name: 'Beta Program', eligibility: 'ineligible' })
      await insertTestProgram({ name: 'Charlie Program', eligibility: 'eligible' })
    })
