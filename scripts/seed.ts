...
function generateTwitterHandle(): string {
  const options = [
    ...
    `@${faker.company.name().toLowerCase().replace(/[^a-z0-9]/g, '')}`,
...
function generateFakeProgram() {
  const assetType = faker.helpers.arrayElement(['web', 'mobile'] as AssetType[])
  const eligibility = faker.helpers.arrayElement(['eligible', 'ineligible'] as EligibilityType[])
...
async function seedPrograms() {
  try {
    ...
    const fakePrograms = Array.from({ length: numberOfPrograms }, generateFakeProgram)
    ...
    for (let i = 0; i < fakePrograms.length; i += batchSize) {
      ...
      console.log(`Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(fakePrograms.length / batchSize)}`)
    }
