...
export default function ProgramPage() {
  ...
  const handlePageChange = useCallback(async (newPage: number) => {
    ...
    await loadPrograms(currentSort.field, currentSort.order, newPage, pagination.limit)
  }, [loadPrograms, currentSort, pagination.limit])

  useEffect(() => {
    loadPrograms(currentSort.field, currentSort.order, pagination.page, pagination.limit)
  }, [loadPrograms, currentSort.field, currentSort.order, pagination.page, pagination.limit])

  const handleSubmit = useCallback(async (data: CreateProgramFormDataType) => {
    try {
      ...
      setShowDialog(false)
      await loadPrograms(currentSort.field, currentSort.order, pagination.page, pagination.limit);
    } catch (err) {
  ...
  const handleDeleteRequest = useCallback((programId: string) => {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => loadPrograms(currentSort.field, currentSort.order, pagination.page, pagination.limit)}>Try Again</Button>
        </div>
      </div>
    )
  }
