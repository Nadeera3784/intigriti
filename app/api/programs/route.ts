...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (sortBy) {
      case "name":
        orderBy =
          sortOrder === "asc" ? asc(programs.name) : desc(programs.name);
        break;
      case "startDate":
        orderBy =
          sortOrder === "asc"
            ? asc(programs.startDate)
            : desc(programs.startDate);
        break;
      case "eligibility":
        orderBy =
          sortOrder === "asc"
            ? asc(programs.eligibility)
            : desc(programs.eligibility);
        break;
      case "createdAt":
      default:
        orderBy =
