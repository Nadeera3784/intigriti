...
const EligibilityTable: React.FC<RelatedProgramsTableProps> = ({ relatedPrograms, handleRemoveRelatedProgram}) => {
  return (
        ...
        <table className="w-full">
          <thead className="bg-muted/25">
            <tr>
              ...
              <th className="text-left p-3 font-medium text-sm">Asset Identifier</th>
          ...
          <tbody>
            {relatedPrograms.map((program) => (
              <tr key={program.id} className="border-t hover:bg-muted/25">
                <td className="p-3">
                  {program.type === "web" ? <Globe className="h-4 w-4" /> : <Smartphone className="h-4 w-4" />}
                </td>
                <td className="p-3 font-mono text-sm truncate">{truncate(program.identifier, 10)}</td>
                <td className="p-3 text-sm">
                  {program.description ? (
                    <span className="line-clamp-2 text-sm truncate">{truncate(program.description, 10)}</span>
                  ) : (
